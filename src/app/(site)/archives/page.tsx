...
export default async function ArchivesPage() {
  ...
  return <>
    <div className="mobile-home-page md:hidden"><MobileFeed items={items} authorName={site.author} authorAvatar={getAuthorAvatar(siteSettings) || undefined} authorAvatarNoBorder={siteSettings.author_avatar_no_border === "1"} /></div>
    <div className="mx-auto hidden max-w-[860px] py-8 md:block md:py-12">
      ...
      <div className="divide-y divide-divider">
        {items.map((item) => item.type === "post" ? (
          ...
          <div key={`moment-${item.value.id}`} className="py-6"><MomentEntry moment={item.value} commentCount={item.commentCount} metrics={item.metrics} authorName={site.author} authorAvatar={getAuthorAvatar(siteSettings) || undefined} authorAvatarNoBorder={siteSettings.author_avatar_no_border === "1"} initialLiked={item.initialLiked}><CommentSection targetType="moment" targetId={item.value.id} /></MomentEntry></div>
        ))}
      </div>
    </div>
  </>;
