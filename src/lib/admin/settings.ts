...
/**
 * 站点设置与分类/标签的业务核心。这里不做任何传输层鉴权：
 * Server Action 负责 Cookie 会话校验，已鉴权的 Route Handler 直接调用本模块。
 */
...
export async function updateSiteSettings(values: Record<string, string>): Promise<SettingsActionResult> {
...
export async function createCategoryByName(name: string): Promise<SettingsActionResult> {
...
export async function updateCategoryById(id: number, name: string): Promise<SettingsActionResult> {
...
export async function deleteCategoryById(id: number): Promise<SettingsActionResult> {
  if (!Number.isInteger(id) || id < 1) return { ok: false, error: "分类不存在" };
  if (!listCategories().some((category) => category.id === id)) return { ok: false, error: "分类不存在" };
  deleteCategory(id);
  revalidateTaxonomy();
...
export async function renameTag(oldTag: string, newTag: string): Promise<SettingsActionResult> {
...
export async function deleteTag(tag: string): Promise<SettingsActionResult> {
