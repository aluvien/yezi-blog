...
function markdownInline(node: Node): string {
  ...
  if (tag === "img") {
    ...
    // 不把原图 alt 交给文字摘要或阅读正文；图片本身通过本地缓存 URL 单独渲染。
...
function tableCellMarkdown(element: Element): string {
  return normalizeMarkdown(Array.from(element.childNodes).map(markdownInline).join(""))
    .replace(/\|/g, "\\|")
    .replace(/\n/g, "<br>");
}
...
function tableMarkdown(element: Element): string {
  const rows = Array.from(element.querySelectorAll("tr")).map((row) =>
    Array.from(row.children).filter((cell) => /^(?:th|td)$/i.test(cell.tagName)).map(tableCellMarkdown),
...
/** 把净化并完成图片本地化的阅读 HTML 转为 GFM Markdown，供新旧归档统一渲染。 */
