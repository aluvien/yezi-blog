...
export default function AdminReferenceAddButton() {
  ...
  function selectReference(selection: { marker: string; snapshot: ArticleReferenceSnapshot } | null) {
    menuRef.current?.removeAttribute("open");
    if (!selection) return;
    setMessage("");
    setError("");
    startTransition(async () => {
      const result = await saveReferenceLibraryAction(selection.snapshot);
      if (!result.ok) {
  ...
  return (
    <>
      <details ref={menuRef} className="admin-reference-add-menu relative shrink-0">
        ...
        <ArticleReferenceDialog onClose={selectReference} />
      </details>
