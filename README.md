...
Bot 还会在同一 Node 进程中每约 3 秒读取一次管理员指令，不需要公网 Webhook：向 Bot 发送 `/qqlogin` 可接收 QQ 音乐授权二维码，扫码成功后会自动保存登录会话；音乐异常通知内也有同样的二维码按钮。新评论通知可在 Telegram 内直接点击“通过”，或点击“回复并通过”后直接发送回复内容，Bot 会自动写入回复并审核通过。后台“设置 → Telegram 通知”可单独关闭新评论推送。
